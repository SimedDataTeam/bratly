...
from bratly import EntityAnnotation, AnnotationCollection, Document, DocumentCollection
from bratly_io_fs import (
    read_and_load_ann_file,
    read_and_load_txt_file,
...
def main():
    ...
    assert(type(annot_col) == AnnotationCollection)
    print(annot_col)
    ...
    annot_col = read_and_load_ann_file(
        "data/demo/ann/myannfile.ann", sorting=False, no_duplicates=False
    )
    assert(type(annot_col) == AnnotationCollection)
    print(annot_col)
    print(annot_col.annotations)
    ...
    document.add_annotation_collection(
        annot_col, version="0.0.2", comment="Sorted version"
    )
    ...
    assert(type(doc_collect) == DocumentCollection)
    print(doc_collect)
    print(
    ...
    mon_dico = doc_collect.stats_annotation_types(verbose=True)
    mon_dico = doc_collect.stats_labels_given_annot_type(
        verbose=True, descendant_type=EntityAnnotation
    )
    mon_dico = doc_collect.stats_entity_contents_given_label(
        verbose=True, label="DET"
    )
    ...
    write_ann_files_in_folder(
        doc_collection=doc_collect, path=join(output_test, "generated_ann_files")
    )
